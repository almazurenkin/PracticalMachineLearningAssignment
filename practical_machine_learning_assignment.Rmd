...
Researches that analyze the collected data are mainly foused on quantifying *how much*
of a particular activity is done and distiguish *what type* of acitivty was performed.
At the same time only few reasearches address a question *how well* the activity was done.
...
```{r, cache=TRUE}
datasetDir <- "./data/HAR Quality Data/"  # String constant with path to the data set directory
...
Note that 'classe' varible in training data set is our classification; and it's missing in testing set.
...
2. We also see that some variables have "#DIV/0!" and "" as a value, these should be 

3. Finally, let's choose only variables, that contain "_belt", "_arm", "_dumbbell", "_forearm",   
We'll exlude "user_name" as unnecessary information of our classifier and include "classe" classifiaction.

```{r}
train.data[train.data == "#DIV/0!" | train.data == ""] <- NA; 

# Count percentace of NA's per column.
nap <- c();
...
Random forests is recognized as one of best performing prediction algorythms, let's apply it here.
...
Let's see how the model performs on training and probe subsets of the training set.

```{r}
T.predictions <- predict(rf_fit, newdata = T)
confusionMatrix(T.predictions, T$classe)
...
Now it's time to veryfy built predictiove model on the test set.
